...
class pb_backupbuddy_mysqlbuddy {
	...
	public function dump( $output_directory, $tables = array(), $rows_start = 0 ) {
		...
		pb_backupbuddy::status( 'details', "mysqlbuddy: Output directory: `{esc_url( $output_directory )}`." );
		if ( count( $tables ) == 1 ) {
	...
	private function _dump_commandline( $output_directory, $tables, $rows_start = 0 ) {
		...
		pb_backupbuddy::status( 'sqlFile', basename( esc_url( $output_file ) ) ); // Tells status checker which file to request size data for when polling server.
		...
		*/
		...
		if ( '0' == $exec_exit_code ) {
			...
			if ( file_exists( $output_file ) ) { // SQL file found. SUCCESS!
				...
				pb_backupbuddy::status( 'details', 'Final SQL database dump file size of `' . basename( esc_url( $output_file ) ) . '`: ' . $sql_filesize . '.' );
				pb_backupbuddy::status( 'finishTableDump', '' );

				return true;
			} else { // SQL file MISSING. FAILURE!
				pb_backupbuddy::status( 'error', 'mysqlbuddy: Though command reported success database dump SQL file is missing: `' . esc_url( $output_file ) . '`.' );
				return false;
			}
		} else {
	...
	private function _dump_php( $output_directory, $tables, $resume_starting_row = 0 ) {
		...
		foreach ( $tables as $table_key => $table ) {
			...
			pb_backupbuddy::status( 'details', 'SQL dump file `' . esc_url( $output_file ) . '`.' );
			if ( '-1' == $this->_maxExecutionTime ) {
			...
			if ( false === ( $file_handle = fopen( $output_file, 'a' ) ) ) {
				pb_backupbuddy::status( 'error', 'Error #9018: Database file is not creatable/writable. Check your permissions for file `' . esc_url( $output_file ) . '` in directory `' . esc_url( $output_directory ) . '`.' );
				return false;
			}
			...
			while ( true === $rows_remain ) {
				...
				if ( false === $rows_remain ) {
					...
					if ( ( microtime( true ) - $last_file_size_output ) > $output_file_size_every_max ) { // It's been long enough to get the current file size of SQL file.
						// Display final SQL file size.
						$sql_filesize = pb_backupbuddy::$format->file_size( filesize( $output_file ) );
						pb_backupbuddy::status( 'details', 'Current database dump file `' . basename( esc_url( $output_file ) ) . '` size: ' . $sql_filesize . '.' );
						$last_file_size_output = microtime( true );
					}
			...
			if ( isset( $output_file ) ) {
				...
				pb_backupbuddy::status( 'details', 'Database SQL dump file (' . basename( esc_url( $output_file ) ) . ') size: ' . pb_backupbuddy::$format->file_size( $stats['size'] ) );
		...
		if ( false === $results ) {
			if ( true !== $ignore_existing ) {
				...
				if ( $this->_9010s_encountered > $this->_max_9010s_allowed ) { // Too many, don't show more.
					...
					pb_backupbuddy::status( 'error', 'Error #9010: Unable to import SQL query. Error: `' . esc_url( $mysql_error ) . '` Errno: `' . $mysql_errno . '`.' );
					if ( false !== stristr( $mysql_error, 'server has gone away' ) ) { // if string denotes that mysql server has gone away bail since it will likely just flood user with errors...
						...
						pb_backupbuddy::status( 'details', 'Last query attempted: `' . esc_url( $query ) . '`.' );
