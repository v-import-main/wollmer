...
class backupbuddy_core {
	...
	public static function alert_core_file_excludes( $excludes ) {
		...
		foreach ( $warn_dirs as $warn_dir ) {
			if ( in_array( $warn_dir, $excludes ) ) {
				$return_array[ 'excluding_corefile-' . md5( $warn_dir ) ] = 'Warning: You are excluding one or more WordPress core or BackupBuddy directories `' . esc_attr( $warn_dir ) . '` which may result in an incomplete or malfunctioning backup. Remove exclusions or backup with another profile or method to avoid problems.';
			}
		}
	...
	public static function get_dat_file_array( $dat_file ) {
		...
		pb_backupbuddy::status( 'details', 'Successfully loaded backup dat file `' . esc_html( $dat_file ) . '`.' );
	...
	public static function get_mysqldump_additional( $includes_or_excludes, $profile, $override_prefix = '' ) {
		if ( ! in_array( $includes_or_excludes, array( 'includes', 'excludes' ), true ) ) {
			...
			error_log( 'BackupBuddy ' . esc_html( $error ) );
			echo esc_html( $error );
			return false;
		}
	...
	public static function calculate_tables( $base_dump_mode, $additional_includes = array(), $additional_excludes = array() ) {
		...
		pb_backupbuddy::status( 'details', 'Base database dump mode (before inclusions/exclusions): `' . esc_html( $base_dump_mode ) . '`.' );
		...
		if ( 'all' === $base_dump_mode ) { // All tables in database to start with.
			...
			if ( ( null === $results ) || ( false === $results ) ) {
				pb_backupbuddy::status( 'error', 'Error #8493894a: Unable to calculate database tables with query: `' . esc_html( $sql ) . '`. Check database permissions or contact host.' );
			}
