...
if ( ! function_exists( 'woocommerce_order_review_inner' ) ) {
	 ...
	 * @param bool $deprecated Deprecated param.
	 */
	function woocommerce_order_review_inner( $deprecated = false ) {
		wc_get_template(
			'checkout/review-order-inner.php',
			array(
				'checkout' => WC()->checkout(),
			)
		);
...
if ( ! function_exists( 'woocommerce_checkout_payment_content' ) ) {

    /**
     * Output the Payment Methods on the checkout.
     */
    function woocommerce_checkout_payment_content() {
        if ( WC()->cart->needs_payment() ) {
            $available_gateways = WC()->payment_gateways()->get_available_payment_gateways();
            WC()->payment_gateways()->set_current_gateway( $available_gateways );
        } else {
	...
	function woocommerce_form_field( $key, $args, $value = null ) {
		...
		if ( ! empty( $field ) ) {
			...
			if ( $args['label'] && 'checkbox' !== $args['type'] ) {
			    if ( wp_kses_post( $args['label'] ) != "Область / район") {
